...
import { SET_OVTOKEN } from '../store/openvidu';
...
const RoomCreate = (props) => {
      ...
      getToken(title).then((token) => {
        ...
        message
          ...
          : axios({
              ...
              .then((res) => {
                console.log(OVState);
                // console.log(OVState.token);
                if (isPrivate) {
                  axios({
                    url: `https://i8a804.p.ssafy.io/api/room/enter/${res.data.result.roomId}/${user.id}`,
                    method: 'POST',
                    data: {
                      roomPrivate: isPrivate,
                      roomPassword: password,
                    ...
                    .then((response) => {
                      if (response.status === 200 || 202) navigate(`${res.data.result.roomId}`);
                    })
                    .catch((err) => {
  ...
  function createSession(title) {
    return new Promise(async (resolve, reject) => {
      let data = JSON.stringify({ customSessionId: title });
      await axios
  ...
  return (
    ...
    >
      <div className={styles.roomCreate}>
        ...
        <button onClick={createRoom} className={styles.roomCreateBtn}>
          방 만들기
        </button>
