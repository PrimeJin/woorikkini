...
function Form() {
  ...
  const onEmailClick = (event) => {
    ...
    if (Email === '') {
      ...
      axios({
        url: `http://i8a804.p.ssafy.io/api/user/email/check?authCodeUserEmail=${Email}`,
        method: 'GET',
      })
        .then((res) => {
  ...
  const onCodeClick = (event) => {
    ...
    if (Code === '') {
      ...
      axios({
        url: `http://i8a804.p.ssafy.io/api/user/email/check?authCodeContent=${Code}&authCodeUserEmail=${Email}`,
        method: 'POST',
      })
        .then((res) => {
          console.log(res.data.message);
  ...
  const onNickCheck = (event) => {
    ...
    if (Nickname === '') {
      ...
      axios({
        url: `http://i8a804.p.ssafy.io/api/user/${Nickname}`,
        method: 'GET',
      })
        .then((res) => {
          console.log('??', res.data.message);
  ...
  const onSubmit = (event) => {
    ...
    if (Email == '' || Password == '' || Name == '' || Nickname == '' || Date == '' || Gender == '') {
      ...
      axios({
        url: 'http://i8a804.p.ssafy.io/api/user',
        method: 'POST',
        data: userData,
      })
        .then((res) => {
